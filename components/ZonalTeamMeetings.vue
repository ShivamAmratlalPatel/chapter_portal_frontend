...
async function onCellEditComplete(event) {
    try {
        await useZonalTeamMeetingsStore().saveZonalTeamMeeting(event.newData.id, event.newData.zone, event.newData.meeting_date, event.newData.agenda, event.newData.minutes_link);
