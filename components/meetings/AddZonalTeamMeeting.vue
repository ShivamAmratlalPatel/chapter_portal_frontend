...
async function save() {
    ...
    visible.value = false;
    try {
        await useZonalTeamMeetingsStore().postZonalTeamMeetings(props.zone, date.value.toISOString().slice(0, 10), agenda.value, minutes_link.value);
        toast.add({
            severity: 'success',
            summary: 'Success',
...
<template>
    ...
    <div class="card flex justify-content-center">
        <Button label="Add Team Meeting" @click="visible = true" />
        <Dialog v-model:visible="visible" modal header="Add Team Meeting" :style="{ width: '25rem' }">
            <span class="p-text-secondary block mb-5">Add a team meeting.</span>
